<template>
  <div>
    <div class="container mx-auto relative">
      ...
      <div>
        <h2 class="py-5 text-xl text-left font-bold">请选择要创建的文书</h2>
        <ul class="wenshuul">
          <li v-for="(item,index) in wenjuanType" :key="index" @click="addNewWenJuan(item.num)">{{item.name}}</li>
        </ul>
      </div>
...
export default {
  data () {
    return {
      wenjuanType: [
        ...
        {name:'抚养权起诉状',num: 21},
        {name:'抚养费起诉状',num: 22},
        {name:'分家析产纠纷起诉状',num: 23},
        {name:'民间借贷起诉状',num: 24},
        {name:'离婚后财产纠纷起诉状',num: 27},
        {name:'调查取证申请书',num: 16},
        {name:'保全申请书',num: 20},
        {name:'证人出庭作证申请书',num: 25},
        {name:'评估申请书',num: 26},
        {name:'笔迹鉴定书',num: 28},
        {name:'精神状态鉴定书',num: 29}
      ],
  ...
  methods: {
    ...
    addNewWenJuan (e) { // 新增问卷
      this.questionnaireSelecter = e;
      console.log(e)
...
<style scoped>
.wenshuul{display: flex;justify-content: space-around;flex-wrap: wrap;}
.wenshuul li{width: 20%;text-align: center;padding: 5px 0;font-size: 16px;color:#343434;border: 2px solid #333333;margin:10px;background: #f1fafb;}
.wenshuul li:hover{color:#ef4e11;background-color:#cde9ec;cursor: pointer;}
