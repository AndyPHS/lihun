                    ...
                    <div v-for="(item,index) in aa[mo.part]" :key="index">
                      ...
                      <div>
                        <div v-if="mo.title== '申请人信息' " class="text-right flex justify-end">
                          <div class="ml-1 mb-3 py-1 text-base text-blue-500 px-1 rounded border border-1 hover:bg-green-500 hover:text-white cursor-pointer" @click="userDeleteSelectAnswerAction(3523,index)">删除申请人信息</div>
                        </div>
                      </div>
                    </div>
                    <div>
                      <div v-if="mo.title== '申请人信息' " class="text-right flex justify-end">
                        <div class="ml-1 mb-3 py-1 text-base text-blue-500 px-1 rounded border border-1 hover:bg-green-500 hover:text-white cursor-pointer" @click="userAddSelectAnswerAction(3523)">添加申请人信息</div>
                      </div>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  data () {
    return {
      ...
      IsShow: false,
      mokuai: [
        {title: '申请人信息', part: 'shenqinginfo', id: 1,num:3523},
        {title: '申请事项', part: 'shenqingshixiang', id: 2,num:3532},
        {title: '事实与理由', part: 'shishiyuliyou', id: 3,num:3533},
        {title: '申请法院', part: 'shenqingfayuan', id: 4,num:3534}
      ],
      active: 0,
  ...
  methods: {
    getshenqinginfo () { // 获取申请人信息模块数据
      returnQuestionnaireJson({'qpid': 3523}).then((data) => {
    ...
    getshenqingshixiang(){ // 获取申请事项模块数据
      returnQuestionnaireJson({'qpid': 3532}).then((data) => {
    ...
    getshishiyuliyou(){ // 获取事实与理由模块数据
      returnQuestionnaireJson({'qpid': 3533}).then((data) => {
    ...
    getshenqingfayuan () {
      returnQuestionnaireJson({'qpid': 3534}).then((data) => { // 查询申请法院模块数据
    ...
    next () {
      ...
      if (this.mokuai[this.active].title == '申请人信息') {
        localStorage.setItem('qpid', 3523)
        demoYanZheng({
          qpid: 3523
        }).then((data) => {
          if (data.data.status_code == 330) {
        ...
        localStorage.setItem('qpid', 3532)
        demoYanZheng({
          qpid: 3532
        }).then((data) => {
          if (data.data.status_code == 330) {
        ...
        localStorage.setItem('qpid', 3533)
        demoYanZheng({
          qpid: 3533
        }).then((data) => {
        ...
        localStorage.setItem('qpid', 3534)
        demoYanZheng({
          qpid: 3534
        }).then((data) => {
          if (data.data.status_code == 330) {
