...
<script>
    ...
    export default {
        ...
        methods:{
            getSelectUqContent(){  //获取文本内容
                selectUqContent().then((data)=>{
                    ...
                    if(data.data.status_code == 200){
                       ...
                       console.log(typeof data.data.data.content)
                    }else{
                        this.$message({
                          message: '获取获取文本失败',
                          type: 'error'
            ...
            AddBoxOk(item, index){
              var that = this;
                if(that.contentMsg.content == that.chooseLabel.addTextareaMsg){
                    that.Content.UqContent.push({
                        status: 1,
                        content: that.contentMsg.content
                    })
                    that.daligeAddBox = false;
                    that.contentMsg.content = '';
                }else{
                   localStorage.setItem('qlid', that.keyWord.flid)
                   AddLabelContent({
                       content: that.contentMsg.content
                   }).then((data)=>{
                        that.contentMsg.content = '';
